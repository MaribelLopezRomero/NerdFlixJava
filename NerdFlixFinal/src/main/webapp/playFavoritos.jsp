<!-- PÁGINA VISALIZAR DATOS DE FAVORITOS -->
...
<head>
	...
	<title>PlayFavoritos - NerdFlix</title>
...
.dropdown dropend {
	...
	<!-- MENÚ SUPERIOR - NAVBAR -->
		<nav class="navbar navbar-expand-sm navbar-dark d-flex justify-content-between">
			...
			<ul class="navbar-nav">
				...
				<li class="nav-item"><a href="favoritos.jsp" class="nav-link active">Favoritos</a>
				</li>
				</li>
			...
			<div class="d-flex justify-content-between">       		
				...
				<!-- CÓDIGO JAVA - Para que aparezca el boton de Administrador siempre que se inicie sesion con admin@gmail.com||administrador -->
				<%
 ...
 <!-- CÓDIGO JAVA - Genera el carrousel -->
	<%
 	//DECLARACIONES
	String param = request.getParameter("param");
	      
	//lista con todas las Favoritos
	LinkedList<Titulos> listaFavoritos = Controller.getFavoritosVer(miUsu.getCod_usuario());
	
	
	//BUCLE FOR - para encontrar que Favoritos hemos pinchado y se muestre en la página
	for(int i=0; i< listaFavoritos.size(); i++){
	
		//BUCLE IF - con el que igualamos le cod_titulo con el parametro generado para visualizar los datos de la Favoritos
		if (listaFavoritos.get(i).getCod_titulo().equals(param)){
		
			out.println(" <div class='container p-5 my-5 border bg-white text-dark'>");
				out.println("<h2 class='text-dark'>"+ listaFavoritos.get(i).getNombre() +"</h2>");
				...
				out.println("  <button type='button' name='button' class='btn btn-success dropdown-toggle' data-bs-toggle='dropdown'>Temporada</button>");
					out.println(" <ul class='dropdown-menu text-white'>");
						...
						String n_temporadas = listaFavoritos.get(i).getTemporadas();
					...
					out.println("<form action='ServletForm' method='POST'>");
						out.println("<button type='submit' name='favoritos' value='"+listaFavoritos.get(i).getCod_titulo()+"' class='btn btn-danger'><img src='images/corazon.png' width='20' height='20'></button>");
							out.println("<input  type='hidden' id='param2' name='param2' value='favorito' />");
						out.println("</form>");
				out.println(" </div>");
				...
				out.println("<table>");
					out.println("<tr>");
						out.println("<td rowspan='4' border='1' width='25%' height='50px' style='vertical-align:middle; text-align='left'>");
							out.println("<img width='300' height='400' align='center' src= " + listaFavoritos.get(i).getImagen()  +">");
						out.println("</td>");
						out.println("<td class='column'width='17%' height='50px' align=''> Fecha de estreno </td>");
						out.println("<td>");
						out.println(listaFavoritos.get(i).getFecha_estreno());
						out.println("</td>");
					out.println(" </tr>");
				
					out.println(" <tr>");
						out.println("<td class='column'width='17%' height='50px' align=''>Género</td>");
						out.println("<td class='column'width='17%' height='50px' align=''>"+ listaFavoritos.get(i).getGenero().toUpperCase() + "</td>");
				    out.println(" </tr>");
			 
				  	out.println(" <tr>");
						out.println("<td class='column'width='17%' height='50px' align=''>Sinopsis:</td>");
						out.println(" <td> <p class='parrafo'>"+ listaFavoritos.get(i).getSinopsis()+"</p></td>");
					out.println(" </tr>");
			 
				 	out.println(" <tr>");
					 	out.println(" <div class='dropdown dropend' style='text-align: center'>");
							out.println("<td align='center'>");
								out.println(" <a href="+listaFavoritos.get(i).getLink() + " class='btn btn-success btn-lg btn-block w-50' target='_blank'>Ver trailer</a>");
							out.println(" </td>");
							out.println("<td align='center'>");
								out.println(" <a href='enConstruccion.jsp' class='btn btn-success btn-lg btn-block w-50' target='_blank'>Ver</a>");
							out.println(" </td>");
						out.println(" </div>");
				 	out.println(" </tr>");
