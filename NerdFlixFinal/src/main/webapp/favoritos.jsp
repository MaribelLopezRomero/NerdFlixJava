...
.imagen{
 ...
 height: 300px;
  ...
  <header>
    ...
    <nav class="navbar navbar-expand-sm bg-dark navbar-dark d-flex justify-content-between">
      ...
      <ul class="navbar-nav">
        ...
        <li class="nav-item">
          <a href="peliculas.jsp" class="nav-link">Películas</a>
        </li>
        <li class="nav-item">
          <a href="series.jsp" class="nav-link">Series</a>
        </li>
        <li class="nav-item">
          <a href="#"><i class="fas fa-search sub-nav-logo"></i></a>
        </li>
        <li class="nav-item">
          <a href="" class="nav-link active">Favoritos</a>
        </li>
        <li class="nav-item">
          <a href="" class="nav-link">En Reproducción</a>
        </li>
      </ul>
  ...
  <!--   CODIGO JAVA  -->
     ...
     LinkedList<Titulos> listaFavoritos = Controller.getFavoritosVer(miUsu.getCod_usuario()); //Aqui quedan guardadas todas las series que me traigo con el select
...
//Recorro la lista de las series para ir sacandolas segun su genero y en los slides
    ...
    for (int j=0; j<listaFavoritos.size(); j++){
   
    String genero =  listaFavoritos.get(j).getGenero(); //sacamos el genero de cada serie
    ...
    //out.println("<a href='playSeries.jsp?param="+listaSeries.get(j).getCod_titulo()+">"+ listaSeries.get(j).getNombre() +"</a>");
   
/* out.println("<p class='text-white'> <a href='playSeries.jsp?param="+listaSeries.get(j).getCod_titulo()+ "'/>" + listaSeries.get(j).getNombre() + "</p>");
  out.println("<img src='"+ listaSeries.get(j).getImagen() +"'/>"); */
 

 
 	out.println("<a href = 'playFavoritos.jsp?param="+listaFavoritos.get(j).getCod_titulo()+"&param2=favorito'> <img class='imagen' src='"+ listaFavoritos.get(j).getImagen() +"'/> </a>");
